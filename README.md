...
- **Discord updates can break it.** Most of TrueIgnore filters Discord's data instead of patching its code, so it's fairly resilient. Three small code patches handle the message list, and those can break when Discord changes. If something stops being hidden, open an issue.
